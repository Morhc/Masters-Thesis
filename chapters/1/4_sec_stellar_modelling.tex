...
These equations are solvable in 3D for simulations, to couple them with a nuclear network becomes too computationally expensive to solve.
...
Additionally, 3D simulations are prohibited by the computational cost of evolving a star through its entire evolution for the amount of time necessary.
Because of this, it is necessary to have a 1D approximation to be able to study the whole of stellar evolution.

1D stellar modelling requires the assumption of spherical symmetry.
...
Next, let us impose a fundamental assumption in 1D stellar modelling: hydrostatic equilibrium.
...
%In 1D, the mean square displacement would be given by
%\[
...
Stellar modelling is also concerned with the evolution of the chemical composition of the star.
...
To describe the relationship between the timescales of nuclear burning and convective mixing, we can use the Dämkohler number \citep{dimotakisTurbulentMixing2005}:
\begin{equation}
D_\alpha = \frac{\tau_{\mathrm{mix}}}{\tau_{\mathrm{burn}}}.
\end{equation}
For the MLT scenario, $\tau_{\mathrm{mix}}$ is given by Equation \ref{eq:t_mlt} and $\tau_{\mathrm{burn}}$ is given by the timescale for nuclear burning, which can be approximated as the inverse of the energy generation rate:
\begin{equation}
...
\caption{$\tau_{\mathrm{MLT}}$, $\tau_{\mathrm{react}}$, and $\tau_{\mathrm{scale}}$ for the $15~\Msun$ $Z=0.02$ stellar model from \cite{ritterNuGridStellarData2018} at $\log_{10}(t-t_{\mathrm{end}})\approx-3.85$ (model number 9200). The black line is $\tau_{\mathrm{MLT}}$. The solid red line is $\tau_{\mathrm{react}}$ for \carbon[12], and the jagged red line is $\tau_{\mathrm{scale}}$ for \carbon[12]. Likewise, the green lines are $\tau_{\mathrm{react}}$ and $\tau_{\mathrm{scale}}$ for \oxygen[16], and the purple lines are $\tau_{\mathrm{react}}$ and $\tau_{\mathrm{scale}}$ for \neon[20]. Black circles are provided where $D_\alpha = 1$ for \carbon[12] and \neon[20].}
\label{fig:ritter_timescales}
\end{figure}

Figure \ref{fig:ritter_timescales} shows that both \carbon[12] and \neon[20] experience convective-reactive nucleosynthesis in the O-shell, but that \oxygen[16] does not.
This is because $D_\alpha = \tau_{\mathrm{MLT}}/\tau_{\mathrm{react}} = 1$ and $D_\alpha = \tau_{\mathrm{MLT}}/\tau_{\mathrm{scale}} = 1$ for \carbon[12] and \neon[20] but $D_\alpha < 1$ for \oxygen[16].
The principal species ingested into the shell will experience convective-reactive nucleosynthesis.