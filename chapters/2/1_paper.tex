\section{Abstract}
\oxygen[]-\carbon[] shell mergers in massive stars are a site for producing the \pnucn{} by the \gprn{}, but 1D stellar models rely on mixing length theory, which does not reproduce the velocity profiles found in 3D hydrodynamic simulations.
We investigate how 3D macro physics informed mixing impacts the nucleosynthesis of \pnucn{}.
We post-process the \oxygen[]-shell of the $\mzams=$15~\Msun, $Z=0.02$ model from the NuGrid stellar data set \citep{ritterNuGridStellarData2018}.
Applying a downturn to velocities at the boundary and increasing velocities across the shell \citep{jonesIdealizedHydrodynamicSimulations2017}, we find non-linear, non-monotonic increase in \pnuc{} production with a spread of $0.96$~\dex{}, and find that isotopic ratios can change. 
Reducing \carbon[]-shell ingestion rates \citep{andrassy3DHydrodynamicSimulations2020} suppresses production, with spreads of $1.22{-}1.84$~\dex{} across MLT and downturn scenarios. 
Applying dips to the diffusion profile to mimic quenching events \citep{andrassy3DHydrodynamicSimulations2020} also suppresses production, with a 0.51~\dex{} spread. 
We also analyze the impact of varying all photo-disintegration rates of unstable \nt-deficient isotopes from \selenium[]{--}\polonium[] by a factor of $10$ up and down.
...
Our results demonstrate that uncertainties in mixing arising from uncertain 3D macro physics are as significant as nuclear physics and are critical for understanding \pnuc{} production during \oxygen[]-\carbon[] shell mergers quantitatively. 

\section{Introduction} \label{sec:intro}

The origin of the solar pattern of the 35 stable \pnucn{} has been a long-standing problem in nucleosynthesis.
\cite{burbidgeSynthesisElementsStars1957} first identified these isotopes as a distinct group that were not produced by the $s$ or \rprn, but instead the \pprn{} by $(\pt,\gamma)$, $(\nt,\gamma)$, and $(\gamma,\nt)$ reactions during a Type II supernova.
Since then, a variety of processes and astrophysical sites have been discussed, but no single mechanism has been identified for the production of all \pnucn{}.
...
\cite{frohlichNeutrinoInducedNucleosynthesisA>642006} and \cite{arconesProductionLightelementPrimary2011} found high neutrino fluxes during a supernova can create \selenium[74]--\cadmium[108] by $(\nt,\pt)$, $(\nt, \gamma)$, and $(\pt, \gamma)$ reactions in the $\nu\pt$ process.
\cite{schatzRpprocessNucleosynthesisExtreme1998} suggest that a hydrogen-rich accretion disk around a neutron star could undergo a series of rapid proton captures in the $r p$ process to produce \selenium[74]--\ruthenium[98].
...
\cite{gorielyHedetonationSubChandrasekharCO2002} proposed that a proton-poor and neutron boosted region could undergo proton-captures could produce all \pnucn{} by the $pn$ process during He-detonation of a \carbon[]-\oxygen[] white dwarf's ejected envelope.
\cite{woosleyPprocessesSupernovae1978} argued that all of the \pnucn{} could be produced by a series of $(\gamma, \nt)$ and $(\gamma, \alpha)$ reactions on the $s$- and \rprn{} seeds in the \gprn during explosive \carbon[]- or \oxygen[]-shell burning, and \cite{arnouldPossibilitySynthesisProtonrich1976} argued that hydrostatic \oxygen[]-burning could also be a site.
\cite{rauscherNucleosynthesisMassiveStars2002} found that the \gprn{} could produce the \pnucn{} in massive stars during the core collapse supernova (CCSN), but also prior if the \oxygen[]-shell merges the \carbon[]-shell, but that it underproduced \isotope{}{92,94}{Mo} and \isotope{}{96,98}{Ru}.
\cite{ritterConvectivereactiveNucleosynthesisSc2018} found that the \pnucn{} could be produced during an \oxygen[]-\carbon[] shell merger.
The \gprn{} describes the flow of $(\gamma,\nt)$, $(\gamma,\pt)$, and $(\gamma,\alpha)$ reactions on the stable isotopic seeds that are already present in the shell at temperatures of $1.5 \leq T \leq \natlog{3}{9}~\K$ \citep{rauscherConstrainingAstrophysicalOrigin2013}.
However, the \gprn{} in massive stars underproduces not only the \molybdenum[] and \ruthenium[], but all \pnucn{} with $A=90-130$ \citep{arnouldPprocessStellarNucleosynthesis2003, woosleyNucleosynthesisRemnantsMassive2007} and \cite{woosleyNucleosynthesisRemnantsMassive2007}.
\cite{travaglioTypeIaSupernovae2011} found that the \gpr{} could produce all \pnucn{} of \spr{} material in a Type Ia supernova, without the underproduction of \molybdenum[92,94] and \ruthenium[96,98].
\cite{travaglioTestingRoleSNe2015} found that modifying the distribution of \spr{} material significantly influenced the production of the \pnucn{}, especially the heaviest ones, and that the lightest three were metallicity dependent.
\cite{battinoHeavyElementsNucleosynthesis2020} found that the H-flashes of rapidly accreting white dwarfs undergo the \iprn{} which modifies the seed distribution to produce \pnucn{} with $96 < A < 196$ by the \gprn{} during the subsequent SNIa.
...
The initial seed distribution is important for the \gprn{} as it sets the initial conditions for photo-disintegration and subsequent nucleosynthesis pathways.

Since \cite{burbidgeSynthesisElementsStars1957}, it has been found that not all \pnucn{} in the solar pattern are produced by a single process.
\cite{bisterzoSprocessLowmetallicityStars2011} found that \gadolinium[152], \erbium[164], and \tantalum[180] have significant contributions of $70.5\%$, $75.5\%$, and $74.5\%$ from the \sprn{}.
\cite{dillmannPProcessSimulationsModified2008} argue that \indium[113] and \tin[115] are made by $\beta$-decays after the \rprn{} through isomeric states.
\cite{gorielyPuzzleSynthesisRare2001} argue that $(\gamma,\nt)$ was too weak to produce \lanthanum[138], and instead that it is made by $\nu_e$-capture on \barium[138] during the CCSN, and \cite{arnouldPprocessStellarNucleosynthesis2003} similarly argue \tantalum[180m] could also have $\nu$-induced contributions.
\cite{sieverdingNProcessLightImproved2018} found that the $\nu$-process is important for the nucleosynthesis of \indium[113], \lanthanum[138], and \tantalum[180].

Existing massive star models that include \pnuc{} production are 1D models that rely on mixing length theory (MLT) to describe the mixing of material in convective shells.
Multi-dimensional hydrodynamic simulations of convective \oxygen[]-shell burning predict higher convective velocities than MLT and a gradual downturn to the velocities at the shell boundaries instead of a sharp cutoff like MLT predicts \citep{bazanConvectionNucleosynthesisCore1994, meakinTurbulentConvectionStellar2007,jonesIdealizedHydrodynamicSimulations2017}.
In addition to this, 3D \oxygen[]/\neon[]/\carbon[] shell mergers have large scale non-radially asymmetries and could have \carbon[]-shell ingestion rates significantly lower than those predicted by 1D models \citep{andrassy3DHydrodynamicSimulations2020,yadavLargescaleMixingViolent2020a}.
The 1D models in \cite{ritterNuGridStellarData2018} have \oxygen[]-\carbon[] shell mergers when the upper boundary of the \oxygen[]-shell rapidly changes it entrains \carbon[12] and \neon[20], which burn in the \oxygen[]-shell and flatten the entropy gradient.
In contrast, \cite{rizzutiShellMergersLate2024a} find in their 3D simulations that the bottom boundary of the \carbon[]-shell extends down and subsumes the \oxygen[]-shell. 
\cite{yadavLargescaleMixingViolent2020a} find similarly that 3D mergers occur differently than 1D because entropy is created due to nuclear burning hotspots lowering entropy in the \neon[]-shell and that material being carried in by convective downdrafts into the \oxygen[]-shell raising its entropy.
The shell merger occurs late in the stellar evolution, and the dynamic behaviour of these supernova progenitors are not well-described in 1D models \citep{arnettRealisticProgenitorsCorecollapse2011, mullerStatusMultiDimensionalCoreCollapse2016, yadavLargescaleMixingViolent2020a}.

The \oxygen[]-shell is a convective-reactive environment where mixing and nuclear burning timescales are equal and species are not well-mixed \citep{ritterConvectivereactiveNucleosynthesisSc2018}.
If these timescales are equal, species could either react or advect to another location and react there.
If these reactions release little energy, the flow would not be significantly affected, such as \lithium[7] production by the Cameron-Fowler mechanism \citep{cameronLithiumSPROCESSRedGiant1971,sackmannCreationSuperrichLithium1992}.
However, if there is significant energy released the flow would be modified \citep{dimotakisTurbulentMixing2005}, which occurs during \hydrogen[]-ingestion into \helium[] layer \citep{herwigCONVECTIVEREACTIVEPROTON2011,herwigGLOBALNONSPHERICALOSCILLATIONS2014} or \oxygen[]-\carbon[] shell mergers with violent mixing \citep{andrassy3DHydrodynamicSimulations2020,yadavLargescaleMixingViolent2020a}.

The Damköhler number \citep{dimotakisTurbulentMixing2005} can be used to quantify whether a region is convective-reactive or not.
\begin{equation}
...
Convective regions are regions where $D_\alpha \ll 1$, so the mixing is much faster than the nuclear burning and species are well-mixed across the region.
Convective-reactive regions have $D_\alpha \sim 1$ and the behaviour described above occurs.
Mixing in 1D models is treated as a diffusive process with a coefficient $D_{\mathrm{MLT}} = \frac{1}{3} v_{\mathrm{MLT}}\ell$, where $v_{\mathrm{MLT}}$ is the convective velocity and $\ell$ is the mixing length given by $\ell = \alpha H_p$, with $H_p$ the pressure scale height and $\alpha$ a free parameter \citep{joyceReviewMixingLength2023}.
The mixing timescale is then given by
\begin{equation}
    \tau_{\mathrm{mix}} = \frac{\ell^2}{D_{\mathrm{MLT}}}
\end{equation}
as in \citep{herwigCONVECTIVEREACTIVEPROTON2011}.
The reaction timescale is defined as
\begin{equation}
    \tau_{\mathrm{react}} = \frac{1}{\rho N_A \langle \sigma v \rangle X_j}
\end{equation}
where $\rho$ is the local density, $N_A$ is Avogadro's number, $\langle \sigma v \rangle$ is the thermally averaged reaction rate, and $Y_j$ is the molar abundance of the interacting species.

Although 3D hydrodynamic simulations may not be solving all relevant equations, such as a robust nuclear network, the 1D predictions lack the ability to describe the mixing during an \oxygen[]-\carbon[] shell merger.
This is because in 3D simulations, the mixing of ingested \carbon[]-shell material occurs by convective plumes which are non-radial and result in spherically asymmetric instabilities \citep{meakinActiveCarbonOxygen2006,andrassy3DHydrodynamicSimulations2020, yadavLargescaleMixingViolent2020a}.
Further, 1D simulations do not capture how a merger happens between the \oxygen[]- and \carbon[]-shells.
This is a problem for \pnuc{} production as \cite{robertiGprocessNucleosynthesisCorecollapse2023} found that \pnucn{} with $A > 110$ were largely produced during the merger rather than during explosive burning.
This result was found to be independent of the peak energy of the CCSN \citep{robertiGprocessNucleosynthesisCorecollapse2024b}.
To explore the impact of the uncertainties from MLT to describe the mixing in convective shells, we adopt in this paper a 3D hydrodynamic inspired set of modifed radial mixing profiles to determine the impact of mixing on the \gprn{} in the \oxygen[]-shell as \carbon[]-shell material is entrained.
In addition to this, the impact of nuclear physics in the \oxygen[]-shell will also be explored.

In Section \ref{sec:methods} we present the methods of how the \cite{ritterNuGridStellarData2018} model is post-processed, the way 3D hydrodynamic results are implemented in 1D, and how the nuclear physics impact is determined. 
In Section \ref{sec:convreacflow} we explore the nucleosynthesis pathways of the \gprn{} in the convective-reactive environment. 
Next, in Sections \ref{sec:convdownturnimpact}--\ref{sec:goshimpact} we explore how different mixing details can impact the production of the p-nuclei: Section \ref{sec:convdownturnimpact} explores the impact of gradual downturn to velocities at the lower boundary and boosting velocities in the \oxygen[]-shell, Section \ref{sec:ingestionimpact} considers how lowering the \carbon[]-shell ingestion rate, and Section \ref{sec:goshimpact} looks at the impact of dips in the diffusion profile.
Section \ref{sec:nuclearimpact} explores the impact from the nuclear physics and its relationship to the mixing details.
Finally, in Section \ref{sec:discussion_conclusion} we summarize the results and discuss the implications of this work.
...
All necessary isotopes were included, which amounted to the network of 1470 species, focusing on the \pt-rich side allowing for faster calculations.
...
\caption{Mass fractions of the \pnucn{} from $\log_{10}(t-t_{\mathrm{end}}) /\mathrm{yr}=-3.856$. used for initial \oxygen[]-shell composition and ingested \carbon[]-shell material. Markers are the same as those used in Figure \ref{fig:kippenhahn}.
\label{fig:initial_comp}}
\end{figure}

The stellar structure from the onset of the merger at $\log_{10}(t-t_{\mathrm{end}}) /\mathrm{\yr}=-3.856$.
...
Using Equation 4 from \cite{jonesIdealizedHydrodynamicSimulations2017}, the downturn to the radial velocity profile can be mimiced in 1D.
\begin{equation}
...
An overview of our results in terms of overproduction factors and average spreads of overproduction factors are presented in Table \ref{tab:mixing_scenarios}.
...
\caption{Reaction fluxes $f_{ij}$ for \dysprosium[156] and mass fractions $X_i$ for \dysprosium[156-158] without radiogenic contributions for the MLT scenario with no ingestion after $t=110~\second$. The direction of reactions $i \rightarrow j$ are written as left to right in the legend.
...
\subsection{Impact from a downturn and boosting mixing speeds}\label{sec:convdownturnimpact}
...
Without ingestion of \carbon[]-shell material, the MLT, $1\times$, and $3\times$ scenarios see little to no production, but for the $19\times$ and $50\times$ scenarios there is a significant underproduction of the heavier \pnucn{}.
...
Whether a particular reaction rate is correlated an isotope's final mass fraction along with the strength of the correlation is dependent on the mixing scenario.
...
The results in Section \ref{sec:goshimpact} have a spread of $0.51~\dex$, Section \ref{sec:convdownturnimpact} $0.96~\dex$, and the maximum spread in Section \ref{sec:ingestionimpact} is $1.84~\dex$.
The average spread in production from varying the nuclear physics rates ranges from $0.56~\dex$ to $0.79~\dex$.

\cite{rauscherUncertaintiesProductionNuclei2016} determined uncertainties for the explosive \gpr{} for a $15~\Msun$ and two $25~\Msun$ models with solar metallicity using a Monte Carlo method.
The spread in their 90\% probability interval for the $15~\Msun$ model was $0.61~\dex$, and for the $25~\Msun$ models were $0.63~\dex$ and $0.99~\dex$.
...
Figure \ref{fig:impact_mixing_cases_bars_all} shows the maximum spread for the \pnucn{} across all mixing scenarios considered in this paper, excluding the case of no merger, have an average spread of 2.45~\dex.
...
The exploration done in this paper shows the importance of understanding the mixing conditions, but not all scenarios are equally likely to represent the conditions in a merger.
